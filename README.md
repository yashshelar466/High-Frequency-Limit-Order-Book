...
**Residual caveat, stated plainly:** if a *later* message references one of these seeded synthetic orders by its real (unrecoverable) LOBSTER ID — the venue eventually touching pre-existing liquidity this file never assigned an ID to — that reference resolves as unknown (tallied in the run's reported `unknown_refs` count), and the seeded quantity at that level goes stale. That's an inherent limit of reconstructing a book from a message-only stream that begins mid-session, not a defect in the reconciliation logic. Separately, this reconciles the *visible* book only — hidden executions are excluded by design.
